...
- **Sub-agent dispatch**: use the Agent tool with `subagent_type` matching the slug of one of the 12 MarketBliss agents (e.g. `subagent_type: campaign-strategist`). For multi-perspective synthesis, prefer the `marketing-supervisor` orchestrator over spawning parallel agents directly.
